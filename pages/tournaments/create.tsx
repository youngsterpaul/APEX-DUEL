  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data, error } = await supabase.rpc('create_tournament', {
        ...
        p_max_players: maxPlayers ? parseInt(maxPlayers, 10) : null,
    ...
    <div style={{ background: '#0a0b14', color: '#fff', minHeight: '100vh' }}>
      ...
      <section style={{ maxWidth: 720, margin: '0 auto', padding: '48px 24px 80px' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          <div>
            <label style={labelStyle}>Max players (optional)</label>
            <input type="number" min="2" max="200" value={maxPlayers} onChange={(e) => setMaxPlayers(e.target.value)} style={inputStyle} placeholder="Leave blank for unlimited" />
            <p style={{ fontSize: 12, color: 'var(--muted)', marginTop: 6 }}>
              Once full, no one else can join.
            </p>
          </div>
