...
export default function CreateLeague() {
  ...
  const confirmUsername = async (rawUsername: string) => {
    ...
    try {
      // Persist which username this player is competing under for this league, so other
      // participants see it. Adjust this call to match your schema — e.g. an UPDATE on
      // league_participants where league_id = pendingEvent.id and profile_id = session.user.id,
      ...
      <div style={{ background: '#0a0b14', color: '#fff', minHeight: '100vh' }}>
        ...
        <section style={{ maxWidth: 520, margin: '0 auto', padding: '80px 24px', textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--muted)', fontSize: 14, marginBottom: 24 }}>
            You're competing in this league too. Confirm the username other players will see you as for{' '}
      ...
      <section style={{ maxWidth: 720, margin: '0 auto', padding: '48px 24px 80px' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: 'var(--muted)' }}>
            ...
            I want to compete in this league too
          </label>
          {creatorPlays && (
            <p style={{ fontSize: 12, color: 'var(--muted)', marginTop: -12 }}>
              After you create the league, we'll ask you to confirm the username you'll compete with for {gameLabel}.
            </p>
          )}
